...
class ArticleController {
  async index ({ request }) {
    ...
    if (type !== '') {
      ...
      const articles = await Article.query().where('type', type).orderBy('updated_at', 'desc').paginate(page)
    ...
    const articles = await Article.query().orderBy('updated_at', 'desc').paginate(page)
